...
glcShader *secondShader = NULL;
GLuint vertexbuffer;
GLuint secondVertexBuffer;
int shaderProgram;
int secondShaderProgram;
...
void display(void)
{
    ...
    glUniform3f(getUniLoc(shaderProgram, "userColor"), 1.0, 0.5, 0.0);

    // Set color variable of the second fragment shader
    glUniform3f(getUniLoc(secondShaderProgram, "userColor"), 1.0, 0.0, 0.0);

    if(shader) shader->Use();

    drawTriangle();

    if(secondShader) secondShader->Use();
...
void init (void)
{
    ...
    secondShader = new glcShader("../shaders/simple.vert", "../shaders/simple.frag");
    secondShaderProgram = secondShader->GetProgram();
